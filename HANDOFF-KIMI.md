...
**RODADA 2 (feedback do usuário após testar) — ✅ COMMITADA**:
...
- **Pendente registrado**: mira ADS ainda cruza a manga esquerda levemente (precisa IK com pole-vector no cotovelo); pistoleiros na seleção seguem em pose de 2 mãos (precisa clipe de 1 mão); mão "blocão redonda" dos chars fallback é o fpArm procedural (design — props fundidos no GLB).
- **Em andamento**: pacote de animações profissionais (Mixamo) pros bots via agente — entrega em `public/models/anims/mixamo/` (testar com `?animdir=models/anims/mixamo`).
