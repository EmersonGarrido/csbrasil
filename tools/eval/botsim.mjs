...
for (const id of ids) {
  ...
  if (DUEL) { out.push({ map: id, tirosBot: m('tirosBot'), acertos: m('acertos'), taxaAcerto: m('taxaAcerto'), fracCabeca: m('fracCabeca'), mortesPorMin: m('mortesPorMin'), janelaAteMorrer: m('janelaAteMorrer') }); continue; }
...
if (DUEL) { console.log('MEDIA taxaAcerto', avg('taxaAcerto'), '| fracCabeca', avg('fracCabeca'), '| mortes/min', avg('mortesPorMin'), '| janela ate morrer (s)', avg('janelaAteMorrer')); process.exit(0); }
