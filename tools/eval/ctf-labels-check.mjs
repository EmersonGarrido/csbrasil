/* ============================================================================
   ...
   O QUE ELA EXIGE, construindo cada mapa REGISTRADO no harness (código de produção):
     ...
     · CTFL2  os nomes de monumento de Brasília só aparecem no awp_map;
     · CTFL3  rótulos únicos dentro do mapa (dois "CENTRO" no HUD não orientam ninguém).

   MUTAÇÃO (régua que não morde não existe):
     --mutante=vaza   apaga a declaração do fy_pool_day depois do build — o mapa volta a
...
for (const [id, def] of Object.entries(MAPS)) {
  ...
  if (MUT === 'vaza' && id === 'fy_pool_day') delete W.ctfPoints;
  ...
  if (id !== 'awp_map' && rot.some((l) => BRASILIA.has(l))) {
    falhas.push(`CTFL2 rótulo de Brasília fora do awp_map: ${rot.filter((l) => BRASILIA.has(l)).join(',')}`);
  }
