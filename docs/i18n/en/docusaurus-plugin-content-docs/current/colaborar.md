...
**9 human author identities** sign commits in this branch: `ruben-cytonic`, `Emerson Garrido`, `Ruben`, `Ruben Marcus`, `William Oliveira`, `Juan Versolato Lopes`, `rubenmarcus`, `matheusgb`, `daltonfontes`. Automated identities are excluded. A Git author name is not necessarily one unique person.
...
|---|---|---|---|---|---:|
| `praca_poderes` | Praça dos Três Poderes | rounds | official | `map_brasilia.js` | 1,830 |
| `piscina_treta` | Piscina da Treta | rounds | official | `map_piscina.js` | 810 |
| `loja_h` | Loja H (Estacionamento) | **capture** | official | `map_havan.js` | 1,952 |
| `ferro_velho` | Ferro Velho do Zé | **capture** | official | `map_ferrovelho.js` | 1,888 |
| `quebrada` | Quebrada (Rua do Baile) | **capture** | official | `map_quebrada.js` | 1,599 |
| `posto_treta` | Posto da Treta | **capture** | official | `map_posto.js` | 489 |
| `atacadao_treta` | Atacadão da Treta | **capture** | official | `map_atacadao.js` | 255 |

**7 registered maps** (7 official, 0 community) - 2 open in rounds and 5 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 9 `map_*.js` files on disk, so a file alone does **not** make a map playable.
