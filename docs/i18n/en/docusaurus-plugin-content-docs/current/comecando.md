...
|---|---:|---|
| Game code | 31,105 lines in 43 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **6,521** lines | `wc -l public/js/game.js` |
| `main.js` | 2,585 lines | `wc -l public/js/main.js` |
...
| Maps in the registry | 11 | `MAPS` object in `maps.js` |
| Visual harnesses in HTML | 15 | `git ls-files 'public/*.html' \| wc -l` |
| Harness scripts | 183 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 54 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.146` | `public/js/version.js` and `package.json` (match) |
...
| Maps in the menu | 11 - 2 open in rounds, **9 in capture** | `MAPS` / `ctfMode` |
...
| `penitenciaria` | Penitenciária da Treta | **capture** | `map_penitenciaria.js` | 245 |

**11 registered maps** - 2 open in rounds and 9 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 13 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **109 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
