...
|---|---:|---|
| Game code | 49,086 lines in 96 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 403 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **248 scripts**; the reason behind each one lives in `SCRIPTS.md`.
