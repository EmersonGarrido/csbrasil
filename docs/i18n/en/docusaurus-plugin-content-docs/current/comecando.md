...
|---|---:|---|
| Game code | 48,976 lines in 96 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 393 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 76 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.268` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **246 scripts**; the reason behind each one lives in `SCRIPTS.md`.
