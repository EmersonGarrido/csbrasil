...
| Harness scripts | 152 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
| Pipeline scripts | 46 | `ls tools/*.mjs \| wc -l` |
| Written entry tasks | 26 | `ls docs/issues/[0-9]*.md \| wc -l` |
| Version | `2.0.0-alpha.80` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **78 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
