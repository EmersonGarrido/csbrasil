...
|---|---:|---|
| Game code | 29,222 lines in 38 files | `cat public/js/*.js \| wc -l` |
| `game.js` | **6,533** lines | `wc -l public/js/game.js` |
| `main.js` | 2,390 lines | `wc -l public/js/main.js` |
...
| Maps in the registry | 6 | `MAPS` object in `maps.js` |
| Visual harnesses in HTML | 15 | `ls public/*.html \| wc -l` |
| Harness scripts | 178 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
| Pipeline scripts | 54 | `ls tools/*.mjs \| wc -l` |
| Written entry tasks | 26 | `ls docs/issues/[0-9]*.md \| wc -l` |
| Version | `2.0.0-alpha.115` | `public/js/version.js` and `package.json` (match) |
...
| Maps in the menu | 6 - 2 open in rounds, **4 in capture** | `MAPS` / `ctfMode` |
...
**6 registered maps** - 2 open in rounds and 4 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 8 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **100 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
