...
| Harness scripts | 165 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
| Pipeline scripts | 47 | `ls tools/*.mjs \| wc -l` |
| Written entry tasks | 26 | `ls docs/issues/[0-9]*.md \| wc -l` |
| Version | `2.0.0-alpha.94` | `public/js/version.js` and `package.json` (match) |
...
**5 registered maps** - 2 open in rounds and 3 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 7 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **89 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
