...
|---|---:|---|
| Game code | 25.295 lines in 28 files | `cat public/js/*.js \| wc -l` |
| `game.js` | **6.525** lines | `wc -l public/js/game.js` |
| `main.js` | 1.603 lines | `wc -l public/js/main.js` |
...
| Versioned animation clips | 526 | `git ls-files public/models/anims \| wc -l` |
| Playable characters | 44, in 5 factions | `CHARACTERS` array in `characters.js` |
| Maps in the registry | 5 | `MAPS` object in `maps.js` |
| Visual harnesses in HTML | 12 | `ls public/*.html \| wc -l` |
| Harness scripts | 150 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
| Pipeline scripts | 43 | `ls tools/*.mjs \| wc -l` |
| Written entry tasks | 15 | `ls docs/issues/[0-9]*.md \| wc -l` |
| Version | `2.0.0-alpha.23` | `public/js/version.js` and `package.json` (they match) |
...
| Maps in the menu | 5 — 2 open in rounds, **3 in capture** | `MAPS` / `ctfMode` |
...
| Ranking / `/u/` pages | **OFF — it is a flag, one line brings it back** | `RANKING_ON` in `src/lib/site.ts` |

> Block generated by `node tools/gen-docs.mjs`. Source: `constants of public/js/game.js · RANKING_ON of src/lib/site.ts`
...
`package.json` has **38 scripts**. Several carry a `//nome` key right above them with the reason they exist — that is where the why lives.
...
7. **[Measured state](./estado.md)** — what is green, what is red, and what has changed
