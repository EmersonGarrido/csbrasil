...
:::note O ARCH.md JÁ ESTEVE desatualizado — e o que ficou de aprendizado
```
$ node tools/gen-arch.mjs --check
✗ ARCH1  ARCH.md está DESATUALIZADO em relação ao código.
         game.js tem 5875 linhas; o índice do ARCH.md não bate.
         Rode: npm run arch
```
**RESOLVIDO desde então.** `npm run arch` e `npm run arch:check` existem hoje no
`package.json` da raiz, e o `ARCH.md` foi regenerado: o cheque sai

```
$ npm run arch:check
✓ ARCH1  ARCH.md em dia (game.js 6428 linhas, 228 símbolos)
```

A saída de falha acima fica como exemplo do que o `--check` imprime **quando** o índice
envelhece — e ele envelhece toda vez que alguém mexe em `game.js` sem rodar `npm run arch`.
Cuidado que continua valendo: no CI o passo está com `continue-on-error: true`, então o
cheque roda mas **não bloqueia**. Tirar essa linha é o que o transforma em portão de
verdade.
:::
...
| **MAPAS / MUNDO** | `map.js` `maps.js` `mapprops.js` `map_brasilia.js` `map_havan.js` `map_pool_day.js` `map_pool_ramos.js` `map_ferrovelho.js` |
...
tools/      arnês     139 scripts .mjs/.py — a régua, o portão e as sondas
```
...
Hoje mapas, armas e personagens são **código** (`map_havan.js` tem 1.458 linhas de
