...
**9 pessoas** assinam commit no histórico **desta branch**: `ruben-cytonic`, `William Oliveira`, `Juan Versolato Lopes`, `CS BRASIL Bot`, `csbrasil-deploy-bot`, `github-actions[bot]`, `Emerson Garrido`, `Ruben`, `daltonfontes`. O resto dos commits é assinado por agentes de IA. Branch não é repositório: quem contribuiu num ramo que esta branch não contém **não aparece aqui**.
