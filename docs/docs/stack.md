...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **150** importam Playwright, **58** importam gltf-transform e **10** importam meshoptimizer.
...
`mint-assets.json` registra **64 assets** gerados via Mint (32 `mint-model` · 13 `mint-asset-pack` · 5 `mint-text-to-3d` · 9 `mint-chat` · 4 `local-derivative` · 1 `openrouter-image-concept`), cada um com `assetId`, `chatUrl` e notas do que deu errado na tentativa anterior.
