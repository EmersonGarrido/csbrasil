...
código (ver [Licença, arte e marca](./licenca.md)). O mecanismo de módulos privados
(multiplayer/editor) **não se aplica** a mapas: não existe mapa da comunidade fechado.
:::
...
```js
fy_meumapa: { name: 'Nome no Menu', build: buildMeuMapa, props: MEUMAPA_PROPS, ctfMode: true, community: true, author: 'Seu Nome', authorGithub: 'seu-usuario', desc: 'Uma frase de descrição pro cartaz em tela cheia.' },
```
...
O `id` segue o padrão da casa: minúsculo, prefixo `fy_`, sem acento e sem espaço
(`fy_meumapa`) — ele vira nome de arquivo de preview e aparece em URL.
