...
| Scripts do arnês | 162 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **100** importam Playwright, **36** importam gltf-transform e **4** importam meshoptimizer.
...
npm run check:fast   # node tools/eval/runner.mjs syntax docs:check arch:check audio:check feet:check eval:ctfhud eval:pause eval:ctfround eval:ctfwin eval:spawn eval:regen eval:pegada eval:ctflabels anims:check
```
