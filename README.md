...
|---|---:|---|
| Código do jogo | 27.326 linhas em 33 arquivos | `cat public/js/*.js \| wc -l` |
| `game.js` | **6.288** linhas | `wc -l public/js/game.js` |
| `main.js` | 1.951 linhas | `wc -l public/js/main.js` |
...
| Versão | `2.0.0-alpha.70` | `public/js/version.js` e `package.json` (batem) |
...
npm run check:fast   # node tools/eval/runner.mjs syntax eval:release eval:telemetry eval:identity eval:prune eval:vminspect docs:check arch:check audio:check feet:check eval:vmlabhud eval:ctfhud eval:pause eval:ctfround eval:ctfwin eval:spawn eval:regen eval:pegada eval:ctflabels anims:check anims:merge:check walls:check media:check travessao:check eval:posters
```

`package.json` tem **70 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
