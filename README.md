...
|---|---:|---|
| Código do jogo | 26.839 linhas em 31 arquivos | `cat public/js/*.js \| wc -l` |
| `game.js` | **6.158** linhas | `wc -l public/js/game.js` |
| `main.js` | 1.707 linhas | `wc -l public/js/main.js` |
...
| Scripts do arnês | 163 | `ls tools/eval/*.mjs tools/eval/*.py \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **100** importam Playwright, **37** importam gltf-transform e **4** importam meshoptimizer.
...
`package.json` tem **53 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
