...
|---|---:|---|
| Código do jogo | 31.414 linhas em 44 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **6.583** linhas | `wc -l public/js/game.js` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **0** importam Playwright, **0** importam gltf-transform e **0** importam meshoptimizer.
