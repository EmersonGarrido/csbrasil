...
|---|---|---|---|
| `public/` | o **jogo** | 31 arquivos `.js`, 27.070 linhas · Three.js `r160` vendorizado | ES modules servidos crus, **zero build**, sem dependência de runtime |
| `src/` | o **site** | 17 páginas `.astro`, 17 rotas `/api` · Astro `^7.1.1` | framework é bem-vindo; `service_role` só no servidor |
| `tools/` | o **arnês** | 172 scripts em `tools/eval/`, 46 em `tools/` | node puro: sobe o jogo real sem browser |
...
npm run check:fast   # node tools/eval/runner.mjs syntax eval:release eval:vminspect docs:check arch:check audio:check feet:check eval:ctfhud eval:pause eval:ctfround eval:ctfwin eval:spawn eval:regen eval:pegada eval:ctflabels anims:check anims:merge:check walls:check media:check travessao:check eval:posters
```

`package.json` tem **63 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
