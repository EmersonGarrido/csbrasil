...
|---|---|---|---|
| `public/` | o **jogo** | 31 arquivos `.js`, 27.139 linhas · Three.js `r160` vendorizado | ES modules servidos crus, **zero build**, sem dependência de runtime |
| `src/` | o **site** | 17 páginas `.astro`, 18 rotas `/api` · Astro `^7.1.1` | framework é bem-vindo; `service_role` só no servidor |
| `tools/` | o **arnês** | 175 scripts em `tools/eval/`, 46 em `tools/` | node puro: sobe o jogo real sem browser |
...
`package.json` tem **68 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
