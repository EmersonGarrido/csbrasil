...
npm run check:fast   # node tools/eval/runner.mjs syntax eval:release eval:prune docs:check arch:check audio:check feet:check eval:ctfhud eval:pause eval:ctfround eval:ctfwin eval:spawn eval:regen eval:pegada eval:ctflabels anims:check anims:merge:check walls:check media:check travessao:check
```
