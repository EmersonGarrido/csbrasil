// ============================================================================
// POSTO DA TRETA — posto de gasolina na beira da rodovia, fim de tarde.
//
// Tema NOVO (não repete praça/piscina/estacionamento/ferro-velho/quebrada): um
// posto de combustível de beira de estrada na hora dourada. Leitura limpa em 3
// corredores paralelos (o que a régua pede: espaço legível acima de riqueza):
//   · OESTE  — a loja de conveniência (parede-fundo) + botijões/dumpster de cover
//   · CENTRO — a MARQUISE sobre as ilhas de bomba (o pátio disputado, bandeira MID)
//   · LESTE  — pátio aberto com jersey barriers, fuscas e pilhas de pneu
// Simétrico em z=0 (TIME E ao sul, TIME B ao norte) → CTF balanceada.
//
// Mistura procedural (marquise, bombas, loja, asfalto, postes de luz) com props
// GLB que já temos (kombi, fusca, saveiro, jersey_barrier, sandbags, pilha_pneus,
// botijao_gas, dumpster, quiosque, concrete_roadblock).
//
// Mesmo contrato de build(scene, T) dos outros mapas: devolve root/colliders/
// occluders/groundHeightAt/slowAt/spawns/pickups/ctfPoints/waypoints/bounds.
// COLISÃO: só AABB alinhado (o motor não tem colisor girado — BUG-21). Marquise e
// beirais são collide:false (passam por cima); pilar, bomba, parede e carro colidem.
// ============================================================================
...
export function buildPosto(scene, T) {
  ...
  const MAT = {
    ...
    loja: lam({ color: 0xe4dccb }),
    lojaBanda: lam({ color: 0xc0392b }),
    marquise: lam({ color: 0xd7dbe0 }),
    marquiseBaixo: lam({ color: 0xb8bdc4 }),
  ...
  grafitar({
    id: 'fy_posto', root, T, waypoints: nodes, seed: 4242, passo: 1.9, alcance: 8.5, cobre: 0.05, minLarg: 0.35,
    bandas: [
