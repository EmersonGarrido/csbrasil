// ============================================================================
// OBRAS DA PREFEITURA — canteiro de obra que nunca acaba (paródia).
//
// Uma "reforma" eterna: chão revirado (terra + concreto + asfalto remendado), TAPUMES
// em volta, uma ESTRUTURA MEIO-CONSTRUÍDA no centro (pilares de concreto cru + laje
// parcial + vergalhão), andaimes, montes de entulho e areia, jersey barriers, um
// GUINDASTE de marco, e placas de "OBRA PARADA · PREVISÃO 2050". Simétrico em z=0
// (TIME E ao sul, TIME B ao norte). A treta é a verba que sumiu.
//
// Mistura procedural (estrutura/andaime/tapume/montes) com props (guindaste, entulho,
// jersey barrier, sandbags, dumpster, botijão, caminhão). Mesmo contrato build(scene, T).
// Colisão só AABB. A* pelos vãos entre o cover.
// ============================================================================
...
export function buildObras(scene, T) {
  ...
  /* ---------------- TERRENO ONDULADO com BURACOS de escavação ----------------
     groundHeightAt(x,z): ondulação suave (o chão revirado) + depressões dos buracos. As bordas
     ficam PLANAS (falloff) pra os tapumes/muros assentarem. O jogador sobe/desce; os bots
     desviam dos buracos fundos (waypoint exclui < -1,0 m e corta ladeira > 0,7 m). */
  ...
  /* ---------------- chão revirado: MALHA ONDULADA deslocada por groundHeightAt ---------------- */
  ...
  /* ---------------- TAPUMES em volta (a cara do canteiro) ---------------- */
  ...
  /* ---------------- ESTRUTURA MEIO-CONSTRUÍDA (centro, o marco + cover) ----------------
     Grid de pilares de concreto cru (colidem) + laje parcial a 3 m (passa por baixo) +
     vergalhões saindo do topo. É o miolo disputado — cobertura por cima e cover atrás dos pilares. */
  ...
  /* ---------------- COVER: entulho, montes de areia, canos, blocos, jersey (simétrico) ---------------- */
  ...
  /* ---------------- ARMAS NO CHÃO (pickups, fileira por spawn + centro) ---------------- */
  ...
  place('ak', -10, 0, 0); place('m4', 10, 0, 0);

  /* ---------------- luz (canteiro empoeirado, sol forte) ---------------- */
  ...
  /* ---------------- ENTORNO (bairro/skyline em volta, backdrop sem colisor) ---------------- */
  {
  ...
  /* ---------------- pixação (tapume é a superfície pichada por definição) ---------------- */
  ...
  /* ---------------- spawns (E sul / B norte) + CTF (triângulo) ---------------- */
  const mk = s => [-8, -2, 4, 10].map(x => ({ x, z: (HALF_Z - 4) * s, yaw: s < 0 ? 0 : Math.PI }));
