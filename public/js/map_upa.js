// ============================================================================
// UPA 24H DA TRETA — pronto-socorro lotado, mapa 100% INTERNO (paródia).
//
// Diferente de tudo: prédio FECHADO (teto, sem céu), estilo cs_office — SALAS separadas por
// paredes com PORTAS, ligadas por um corredor central em cruz. Recepção/espera, triagem,
// consultórios, RAIO-X, farmácia, enfermaria e a SALA DE EMERGÊNCIA. Cheio de canto pra se
// esconder (atrás de macas, balcões, armários, nas salas). TIME E nasce na recepção (sul-oeste),
// TIME B na emergência (norte-leste). A treta é a fila eterna: "SENHA 999 · ESPERA 8H".
//
// Tudo procedural (paredes/portas/móveis) + props (manequins=pacientes, gôndola=farmácia, TVs).
// Mesmo contrato build(scene, T). Colisão só AABB. A* pelos corredores e portas.
// ============================================================================
...
export const UPA_PROPS = ['manequim', 'gondola_mercado', 'gondola_eletro', 'painel_tvs', 'caixa_cobranca', 'cooler', 'shopping_cart', 'arara_roupas'];
...
export function buildUpa(scene, T) {
  ...
  const MAT = {
    piso: lam({ map: floorTex() }), parede: lam({ map: wallTex() }), paredeAlta: lam({ color: 0xf2f0ea }),
    teto: lam({ color: 0xe4e7ea }), maca: lam({ color: 0xf2f5f7 }), aco: lam({ color: 0x9aa0a6 }),
  ...
  /* ---------------- piso + teto + luminárias (interno, sem céu) ---------------- */
  ...
  /* ---------------- PAREDES: perímetro + corredor central em cruz + divisórias das salas ---------------- */
  ...
  // corredor central longitudinal (x = ∓3,5) com portas pras salas
  wallZ(-3.5, -wZ, wZ, [[-27, -23], [-2, 2], [23, 27]]);   // portas: recepção · consultórios · enfermaria
  wallZ(3.5, -wZ, wZ, [[-27, -23], [-2, 2], [23, 27]]);    // portas: farmácia · triagem/raio-x · emergência
  ...
  /* ---------------- placas das salas (acima das portas) + letreiro UPA ---------------- */
  ...
  /* ---------------- móveis (helpers) — tudo com collider REAL (senão o corpo entra) ---------------- */
  ...
  /* ---------------- aparelhos médicos (procedural 3D) ---------------- */
  ...
  /* ---------------- RECEPÇÃO/ESPERA (oeste-sul) — Time E nasce em z=-31, faixa limpa ---------------- */
  ...
  /* ---------------- CONSULTÓRIOS (oeste-meio, 2 salas em z=∓7) ---------------- */
  ...
  /* ---------------- ENFERMARIA (oeste-norte) — fileiras de macas + soro ---------------- */
  ...
  /* ---------------- FARMÁCIA (leste-sul) — gôndolas de remédio ---------------- */
  ...
  /* ---------------- TRIAGEM (leste-meio-sul) + RAIO-X (leste-meio-norte) ---------------- */
  ...
  /* ---------------- SALA DE EMERGÊNCIA (leste-norte) — Time B nasce em z=31, faixa limpa ---------------- */
  ...
  /* ---------------- corredor central: bancos, macas de fila e plantas (cobertura + densidade) ---------------- */
  ...
  /* ---------------- avisos da treta pelos corredores ---------------- */
  ...
  /* ---------------- luz interna (fluorescente, sem sol/céu) ---------------- */
  const hemi = new THREE.HemisphereLight(0xf4f8ff, 0xb8c0c8, 1.15); scene.add(hemi);
  ...
  /* ---------------- pixação nas paredes internas (UPA pichada) ---------------- */
  ...
  /* ---------------- spawns: E recepção (oeste-sul) / B emergência (leste-norte) + CTF ---------------- */
  const spawns = {
